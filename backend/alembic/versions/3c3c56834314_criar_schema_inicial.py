...
def upgrade() -> None:
    ...
    sa.ForeignKeyConstraint(['uf'], ['estados.id'], ),
    sa.PrimaryKeyConstraint('id')
    )
    ...
    sa.PrimaryKeyConstraint('id'),
    sa.UniqueConstraint('logradouro', 'numero', 'complemento', 'cep', 'bairro', 'cidade_id')
    )

    # ===== Hierarquia de localização: corredores -> seccoes -> prateleiras -> localizacoes =====
    op.create_table('seccoes',
    sa.Column('id', sa.SmallInteger(), nullable=False),
    ...
    sa.ForeignKeyConstraint(['prateleira_id'], ['prateleiras.id'], ),
    sa.PrimaryKeyConstraint('id'),
    ...
    sa.Column('excluido_em', sa.DateTime(timezone=True), nullable=True),
    sa.Column('excluido_por', sa.BigInteger(), nullable=True),
    sa.ForeignKeyConstraint(['excluido_por'], ['usuarios.id'], ),
    ...
    op.create_foreign_key(
        'fk_funcionario_excluido_por', 'funcionarios', 'usuarios',
        ['excluido_por'], ['id']
    )
    ...
    sa.ForeignKeyConstraint(['unidade_medida_id'], ['unidades_medida.id'], ),
    sa.PrimaryKeyConstraint('id'),
    ...
    op.create_table('registros_saida',
    sa.Column('id', sa.BigInteger(), nullable=False),
    ...
    sa.Column('funcionario_id', sa.BigInteger(), nullable=False),
    sa.ForeignKeyConstraint(['entrada_id'], ['registros_entrada.id'], ),
    sa.ForeignKeyConstraint(['funcionario_id'], ['funcionarios.id'], ),
    sa.PrimaryKeyConstraint('id')
    )
    ...
    op.execute("CREATE INDEX idx_entrada_fornecedor ON registros_entrada(fornecedor_id);")
    op.execute("CREATE INDEX idx_entrada_lote ON registros_entrada(lote_id);")
    op.execute("CREATE INDEX idx_entrada_localizacao ON registros_entrada(localizacao_id);")
...
def downgrade() -> None:
    ...
    op.drop_constraint('fk_funcionario_excluido_por', 'funcionarios', type_='foreignkey')
