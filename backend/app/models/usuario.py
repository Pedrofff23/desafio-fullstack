...
class Funcionario(Base):
    ...
    usuario: Mapped["Usuario | None"] = relationship(
        back_populates="funcionario", foreign_keys="Usuario.funcionario_id"
    )
...
class Usuario(Base):
    ...
    perfil: Mapped[str] = mapped_column(
        String(20), nullable=False, default="funcionario"
    )
    ativo: Mapped[bool] = mapped_column(Boolean, server_default=true(), nullable=False)
    data_cadastro: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), server_default=func.now(), nullable=False
    ...
    funcionario: Mapped["Funcionario"] = relationship(
        back_populates="usuario", foreign_keys="Usuario.funcionario_id"
    )
