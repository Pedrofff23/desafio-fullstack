...
async def _cidade_id(session, ibge: int | None = None, nome: str | None = None) -> int:
    ...
    if ibge:
        cid = await _scalar(session, "SELECT id FROM cidades WHERE ibge = :ibge LIMIT 1", {"ibge": ibge})
    else:
        cid = await _scalar(session, "SELECT id FROM cidades WHERE nome = :nome LIMIT 1", {"nome": nome})
    if cid is None:
...
async def seed_funcionarios_usuarios(session, endereco_ids: list[int], contato_ids: list[int]) -> list[int]:
    """Cria 4 funcionários com seus respectivos usuários."""
    pessoas = [
        ("João Silva",   endereco_ids[0], contato_ids[0], "joao.silva@mercado.com",   "admin"),
        ("Maria Santos", endereco_ids[1], contato_ids[1], "maria.santos@mercado.com", "funcionario"),
        ("Pedro Costa",  endereco_ids[2], contato_ids[2], "pedro.costa@mercado.com",  "funcionario"),
        ("Ana Oliveira", endereco_ids[3], contato_ids[3], "ana.oliveira@mercado.com", "funcionario"),
    ]
    ...
    for nome, end_id, cont_id, email, perfil in pessoas:
        exists = await _scalar(session, "SELECT id FROM usuarios WHERE email = :email", {"email": email})
        if exists:
            func_id = await _scalar(session, "SELECT funcionario_id FROM usuarios WHERE email = :email", {"email": email})
            func_ids.append(func_id)
            logger.info("Usuário '%s' já existe; pulando.", email)
            continue

        func_id = await _scalar(session, """
        ...
        """), {"func_id": func_id, "email": email, "senha_hash": senha_hash, "perfil": perfil})
...
async def seed_fornecedores(session, contato_ids: list[int], endereco_ids: list[int]) -> list[int]:
    fornecedores_data = [
    ...
    for nome_empresa, cont_id, end_id in fornecedores_data:
        ...
        if fid is None:
            fid = await _scalar(session, "SELECT id FROM fornecedores WHERE nome_empresa = :nome", {"nome": nome_empresa})
        ids.append(fid)
...
async def _unidade(session, sigla: str) -> int:
    return await _scalar(session, "SELECT id FROM unidades_medida WHERE sigla = :s", {"s": sigla})


async def _categoria(session, nome: str) -> int:
    return await _scalar(session, "SELECT id FROM categorias WHERE nome = :n", {"n": nome})
...
async def _inserir_produto(session, codigo, nome, descricao, um, cat, loc, func_id):
    pid = await _scalar(session, """
        ...
        RETURNING id
    """, {"c": codigo, "n": nome, "d": descricao, "um": um, "cat": cat, "loc": loc, "func": func_id})
    if pid is None:
        pid = await _scalar(session, "SELECT id FROM produtos WHERE codigo = :c", {"c": codigo})
    return pid


async def _inserir_lote(session, prod_id, numero_lote, dt_prod, dt_val):
    lid = await _scalar(session, """
...
async def _inserir_entrada(session, lote_id, fornec_id, loc_id, qtd, dt_entrada, p_custo, p_sug, func_id):
    ...
    if eid is None:
        eid = await _scalar(session, """
            ...
            RETURNING id
        """, {
            "lote_id": lote_id, "forn_id": fornec_id, "loc_id": loc_id,
            "qtd": qtd, "dt": dt_entrada, "pc": p_custo, "ps": p_sug, "func_id": func_id,
        })
    return eid
...
async def _inserir_saida(session, entrada_id, qtd, dt_saida, p_venda, func_id):
    ...
    if not existe:
        ...
        """), {"eid": entrada_id, "qtd": qtd, "dt": dt_saida, "pv": p_venda, "fid": func_id})
...
async def seed_produtos(session, func_ids: list[int], fornec_ids: list[int]) -> None:
    ...
    c = {n: await _categoria(session, n) for n in (
        "Laticínios", "Bebidas", "Carnes", "Hortifrúti", "Padaria",
        "Bebidas Alcoólicas", "Enlatados", "Congelados", "Doces", "Limpeza",
    )}
    ...
    dados = [
        ...
        ("PROD001", "Leite Integral",         "Leite de vaca integral pasteurizado 1L",     "l",   "Laticínios",       4, 0),
        ("PROD002", "Iogurte Natural",         "Iogurte natural sem sabor 500g",             "un",  "Laticínios",       5, 1),
        ("PROD003", "Refrigerante de Cola 2L", "Refrigerante de cola 2 litros",              "l",   "Bebidas",          1, 2),
        ("PROD004", "Pão Francês",             "Pão francês fresco unidade",                 "un",  "Padaria",          6, 3),
        ("PROD005", "Carne Bovina",            "Carne bovina alcatra congelada por kg",       "kg",  "Carnes",           2, 1),
        ("PROD006", "Leite sem Lactose",       "Leite sem lactose 1L",                       "l",   "Laticínios",       4, 2),
        ("PROD007", "Ovo de Galinha",          "Ovos de galinha caipira caixa 12 unidades",  "cx",  "Hortifrúti",       2, 3),
        ("PROD008", "Cerveja Pilsen 6pk",      "Cerveja Pilsen 6 pack 330ml cada",           "pct", "Bebidas Alcoólicas", 3, 0),
    ]
    for cod, nome, desc, un_s, cat_n, loc_i, f_i in dados:
        p[cod] = await _inserir_produto(session, cod, nome, desc, u[un_s], c[cat_n], locs[loc_i], fn[f_i])
    ...
    for num, fi, qtd, dt, pc, ps, fni in entradas_base:
        e[num] = await _inserir_entrada(session, l[num], f[fi], await loc_do_lote(num), qtd, dt, pc, ps, fn[fni])
    ...
    # ------------------------------------------------------------------
    dados_extra = [
        ("PROD009", "Arroz Branco 5kg",        "Arroz branco tipo 1, pacote de 5 kg",         "pct", "Enlatados",  2, 0),
        ("PROD010", "Feijão Carioca 1kg",      "Feijão carioca selecionado, pacote de 1 kg",   "pct", "Enlatados",  2, 0),
        ("PROD011", "Milho Verde em Conserva", "Lata de milho verde 170 g",                    "un",  "Enlatados",  2, 1),
        ("PROD012", "Batata Frita Congelada",  "Batata palito congelada, pacote de 1,5 kg",    "pct", "Congelados", 5, 1),
        ("PROD013", "Sorvete de Chocolate",    "Sorvete de chocolate, pote de 1,5 L",          "un",  "Congelados", 5, 2),
        ("PROD014", "Banana Prata",            "Banana prata vendida por quilograma",           "kg",  "Hortifrúti", 6, 2),
        ("PROD015", "Maçã Gala",              "Maçã gala vendida por quilograma",             "kg",  "Hortifrúti", 6, 2),
        ("PROD016", "Chocolate ao Leite",      "Barra de chocolate ao leite 90 g",             "un",  "Doces",      2, 3),
        ("PROD017", "Biscoito Recheado",       "Biscoito recheado sabor chocolate 140 g",      "pct", "Doces",      2, 3),
        ("PROD018", "Água Sanitária 1L",       "Água sanitária para limpeza doméstica, 1 L",   "fr",  "Limpeza",    6, 3),
    ]
    ...
    dados_dinamicos = [
        ("PROD019", "Queijo Muçarela Fatiado",  "Queijo muçarela fatiado, embalagem de 200 g", "pct", "Laticínios", 4, 0),
        ("PROD020", "Iogurte de Morango",        "Iogurte sabor morango, pote de 170 g",        "un",  "Laticínios", 5, 1),
        ("PROD021", "Peito de Frango Congelado", "Peito de frango congelado, pacote de 1 kg",   "kg",  "Congelados", 5, 2),
        ("PROD022", "Suco de Laranja Integral",  "Suco de laranja integral, garrafa de 1 L",    "l",   "Bebidas",    1, 3),
    ]
    ...
    for num, prod_cod, dias_p, dias_v, fi, qtd_ent, pc, ps, fni, qtd_sai, pv in lotes_dinamicos:
        dp = hoje - timedelta(days=dias_p)
        dv = hoje + timedelta(days=dias_v)
        l[num] = await _inserir_lote(session, p[prod_cod], num, dp, dv)
        loc_id = await loc_do_lote(num)
        e[num] = await _inserir_entrada(session, l[num], f[fi], loc_id, qtd_ent, "NOW()", pc, ps, fn[fni])

        existe = await _scalar(session, "SELECT 1 FROM registros_saida WHERE entrada_id = :eid LIMIT 1", {"eid": e[num]})
        if not existe:
            await session.execute(text("""
                INSERT INTO registros_saida (entrada_id, quantidade, data_saida, preco_venda, funcionario_id)
